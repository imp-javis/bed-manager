...
class Ui_patientStatus(object):
    def setupUi(self, patientStatus): #, user, phototag, pos
        ...
        self.bedTable.setColumnWidth(self.downstreamColumn,115)
        ...
        self.wardTable.setColumnWidth(self.downstreamColumn,115)
...
#-------------------- functions to record changes ------------------------------
    def displayBed(self):      # this function updates the list of patients currently in an AMU bed that is saved in database
        ...
        # self.bedTable = bedTable, loungeTable
        size = getBedListSize()
        ...
        self.bedTable.setRowCount(size) #setting the self.bedTable row size
        row = 0
        ...
        for pat in listnow: #looping through each row of bed list to display each line/patient
            ...
            # creating widgets for the self.bedTable cells
            ...
            self.bedTable.setItem(row, self.idColumn, item)
            self.bedTable.setItem(row, self.nameColumn, QtWidgets.QTableWidgetItem("{} {}".format(patient[1], patient[2]))) # name
            self.bedTable.setItem(row, self.detailsColumn, QtWidgets.QTableWidgetItem(("Age: {} \nGender: {}").format(patient[3],patient[4]))) #age and gender
            self.bedTable.setItem(row, self.diagnosisColumn, QtWidgets.QTableWidgetItem(("{}").format(patient[5]))) # diagnosis
            self.bedTable.setItem(row, self.isoColumn, QtWidgets.QTableWidgetItem(isoStatus)) # isolation
            self.bedTable.setItem(row, self.bedColumn, QtWidgets.QTableWidgetItem(pat[1])) # bed
            self.bedTable.setItem(row, self.dischargeColumn, checkBox_discharge) # discharge status
            self.bedTable.setItem(row, self.dis_loungeColumn, checkBox_dischargeLounge) # discharge lounge
            self.bedTable.setItem(row, self.dis_sumColumn, checkBox_dischargeSum) # discharge summary
            self.bedTable.setItem(row, self.dis_medColumn, checkBox_dischargeMed) # discharge medication
            self.bedTable.setCellWidget(row, self.downstreamColumn, comboBox_ward) # downstream ward
            self.bedTable.setItem(row, self.deathColumn, checkBox_death) #death
            row = row+1

        # self.bedTable.cellClicked.connect(self.updateDest)
...
#---------------------- confirm button actions ---------------------------------
    ...
    #def confirmChanges(self):
        # for patient in each row:
            # if lounge is checked:

            # if ward is chosen
                # check if discharge/death is chosen
                    # if not, then update database withself.check_ward()
